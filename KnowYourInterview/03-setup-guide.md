...
> You do **not** need any of this on your work laptop. On your personal machine, download `setup-project.sh` (and the four docs), then run `bash setup-project.sh` to recreate the project folder. Then follow along below.
...
- **IntelliJ IDEA Community** (free) — best for the Java/Spring backend. [jetbrains.com/idea](https://www.jetbrains.com/idea/download). Bundles Gradle, so you can skip installing Gradle separately.
...
1. Put `setup-project.sh` and the four docs in a folder on your personal machine.
2. Run:
```bash
bash setup-project.sh
```
This creates `./KnowYourInterview` with the full Phase 1 skeleton.
3. Move the docs in:
```bash
mkdir -p KnowYourInterview/docs
mv 0*.md KnowYourInterview/docs/
```
4. Put it under version control and push to your GitHub:
```bash
...
```bash
cd api
gradle wrapper --gradle-version 8.14   # one-time; creates ./gradlew
...
- **Gradle download blocked / slow** → normal on first run; if it fails entirely, check network/firewall.
