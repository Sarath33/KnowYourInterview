...
- **No email provider.** Password-reset links are logged server-side (`AuthService.forgotPassword`), not emailed — same gap as local dev, just more visible now that this is a real deployed instance. Anyone requesting a reset needs someone with API log access to hand them the link.
