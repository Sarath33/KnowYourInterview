...
| `JWT_SECRET` | *(generated, 256-bit)* | Overrides the `dev-only-change-me...` default in `application.yml`. **Startup fails under the `prod` profile if this is left at the default** — see `ProdSecretGuard`. |
| `PAYOUT_ENC_KEY` | *(generated, base64 32 bytes)* | AES-256-GCM key encrypting the payout UPI VPA at rest (`AesGcmEncryptor`). **Startup fails under the `prod` profile if this is left at the committed dev default** — see `ProdSecretGuard`. Generate with `openssl rand -base64 32`. **Back this up somewhere you won't lose it**: rotating or losing it makes every existing `enc:v1:` payout VPA permanently unreadable — there is no key-versioning yet, only the `v1` prefix left as a seam for it. |
...
| `RESEND_API_KEY` | *(your Resend API key)* | **Recommended on Railway over the `MAIL_*` SMTP vars below.** Railway blocks outbound SMTP — confirmed in production by connect timeouts to `smtp.resend.com` on both port 587 and 465 (`SocketTimeoutException`, before TLS/auth ever ran). Setting this switches `EmailConfig` to `ResendApiEmailSender`, which sends over Resend's HTTPS API (port 443) instead, and takes priority over any `MAIL_HOST` config so a stale SMTP setup can't silently keep failing. Requires `MAIL_FROM_ADDRESS` below too. |
| `MAIL_HOST` | *(your provider's SMTP host)* | Only used if `RESEND_API_KEY` is unset. **Both unset = no email is sent** — messages are written to the API log instead. Fine for a host that doesn't block outbound SMTP; on Railway, use `RESEND_API_KEY` instead. |
| `MAIL_PORT` | `587` | 587 + STARTTLS is the near-universal default. Some providers also offer 465 with implicit TLS. Irrelevant if using `RESEND_API_KEY`. |
| `MAIL_USERNAME` / `MAIL_PASSWORD` | *(provider SMTP credentials)* | For Postmark these are both the Server API token; SendGrid uses the literal username `apikey` plus the key; Resend uses `resend` plus the key. Check your provider's SMTP page. Irrelevant if using `RESEND_API_KEY`. |
| `MAIL_FROM_ADDRESS` | e.g. `no-reply@yourdomain.com` | Must be at a domain the provider has verified for sending, or mail is rejected or spam-filed regardless of everything else being right. Used by both the SMTP and Resend-API paths. |
| `MAIL_FROM_NAME` | `Know Your Interview` | Display name on the From: header. Optional. Used by both the SMTP and Resend-API paths. |
...
- **A missing `prod`-guarded secret looks exactly like a broken healthcheck.** `ProdSecretGuard` is an `ApplicationRunner`, so it throws *after* the context is built and Flyway has run — the build succeeds, migrations apply cleanly, then the process exits and Railway reports `1/1 replicas never became healthy!` with no obvious cause. It has caught two deploys so far (`JWT_SECRET`, then `PAYOUT_ENC_KEY` on 2026-08-02). **Whenever a new secret with a committed dev default gets a guard, add it to the env var table above in the same commit** — the guard is doing its job; the doc drifting is what costs the hour. To diagnose in seconds, filter the deploy logs for `Application run failed`: the ECS `error.message` field names the exact variable.
...
- **Railway blocks outbound SMTP.** Discovered in production: `MAIL_HOST=smtp.resend.com` with both port 587 and 465 timed out at the socket-connect level. Fixed by adding `ResendApiEmailSender`, which sends over Resend's HTTPS API instead — set `RESEND_API_KEY` rather than the `MAIL_*` SMTP vars on Railway. See the env var table above.
