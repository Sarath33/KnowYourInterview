...
| Build tool | Maven | latest | Widely documented; IntelliJ has built-in support. |
...
**Phase 5 — Hardening & observability (1 wk):** tests (unit/integration/E2E), Sentry, logging, health probes, security pass (validation, rate limiting, secrets, dependency scan, OWASP basics). _(Done for web/API, pending local verification: Testcontainers-based integration tests (real Postgres/Redis, run via `mvn verify`), Spring Boot Actuator health/liveness/readiness probes, Redis-backed rate limiting on auth endpoints, Sentry error tracking + ECS structured logging (both wired but inert until configured), OWASP dependency-check plugin (opt-in `mvn -Powasp verify`), and a validation/headers/SQL-injection review that found the existing code already in good shape. No E2E browser test suite yet. See 04-handoff.md.)_

**Phase 5.5 — Production-quality pass, part 1: testing foundation (done, pending local verification).** A deliberate detour before Phase 6, prompted by a "college project vs. production" review of the codebase. Closed the biggest gap found: service-layer business logic (`AuthService`, `ExperienceService`, `AdminReviewService`, `PurchaseService`, `PayoutService`) had no unit tests, only controller slice tests — now covered by ~90 Mockito-based unit tests across all 5. Added a Vitest + React Testing Library harness on the frontend (previously zero tests) with component tests for the auth forms, the Razorpay unlock flow, and the admin approve/reject flow. Added `frontend-ci.yml` so frontend changes get the same lint/typecheck/test/build gate the backend already had. Full list of what the same review flagged as still open, roughly in the order we agreed to tackle them: containerize + get a real deploy pipeline running (folds into Phase 6 below), move proof-document storage off local disk to S3, ship a Terms of Service/Privacy Policy/refund policy, add uptime + payment-failure alerting once something is deployed, then a load-testing and accessibility pass.
